...
const Admin = () => {
  ...
  useEffect(() => {
    document.title = "Admin | BookStore";
  ...
  return (
    <>
      <div className="px-20 py-10 relative">
        {/* Title */}
        <h1 className="uppercase text-8xl font-bold">Admin Dashboard</h1>
              ...
              {books.map((ele: any) => (
                <div key={ele.id} className="grid grid-cols-10">
                  ...
                  <div className="col-span-1 py-2 bg-col-one text-black text-center text-xl font-bold border-b-2">
                    <button
                      onClick={() => navigate("/add-product", { state: { editBook: ele } })}
                      className="px-8 py-1 border-2 shadow-[3px_3px_0px_#000] duration-200 hover:shadow-none hover:translate-0.5 border-black bg-col-five uppercase text-white"
                    >
                      Edit
                    </button>
                  </div>
        ...
        {/* Sticky Add Button */}
        <div className="sticky bottom-8 z-50 flex justify-end mt-10 pointer-events-none">
          <button
            onClick={() => navigate("/add-product")}
            title="Add New Book"
            className="pointer-events-auto translate-x-12 w-20 h-20 flex justify-center items-center bg-col-two rounded-full border-4 border-black shadow-[4px_4px_0px_#000] hover:translate-1 active:translate-1.5 hover:shadow-none duration-300"
          >
            <FaPlus size={34} />
          </button>
        </div>
