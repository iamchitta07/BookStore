        ...
        {data.length !== 0 && (
          <div className="w-[80%] pt-2 pb-4 mb-5">
            {data.map((ele) => (
              <Link
                to={`/books/${ele.id}`}
                className="w-full border-2 border-col-three py-3 px-5 mb-2 flex justify-between shadow-[3px_3px_0px_#FD5A46] hover:shadow-none hover:translate-0.75 duration-200"
                key={ele.id}
              >
                ...
                <h1 className="text-2xl font-bold">
                  Stock Left: <span className="text-col-three">{ele.stock_quantity}</span>
                </h1>
              </Link>
            ))}
          </div>
